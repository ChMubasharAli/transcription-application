...
WITH CHECK (bucket_id = 'dialogue-audio' AND auth.uid()::text = (storage.foldername(name))[1]);

CREATE POLICY "Users can view their own audio responses"
ON storage.objects FOR SELECT 
USING (bucket_id = 'dialogue-audio' AND auth.uid()::text = (storage.foldername(name))[1]);
...
CREATE POLICY "Public can view source audio files"
ON storage.objects FOR SELECT 
USING (bucket_id = 'dialogue-audio' AND (storage.foldername(name))[1] = 'source');

-- Create indexes for better performance
CREATE INDEX idx_user_test_sessions_user_id ON public.user_test_sessions(user_id);
CREATE INDEX idx_user_test_sessions_dialogue_id ON public.user_test_sessions(dialogue_id);
CREATE INDEX idx_user_test_sessions_status ON public.user_test_sessions(status);
CREATE INDEX idx_user_segment_responses_session_id ON public.user_segment_responses(session_id);
CREATE INDEX idx_user_segment_responses_user_id ON public.user_segment_responses(user_id);
CREATE INDEX idx_dialogue_segments_dialogue_id ON public.dialogue_segments(dialogue_id);
CREATE INDEX idx_dialogue_segments_order ON public.dialogue_segments(segment_order);
CREATE INDEX idx_dialogues_domain_id ON public.dialogues(domain_id);
