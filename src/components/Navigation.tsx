        ...
        <div className="flex items-center justify-between h-20 my-0">
          ...
          <div className="flex items-center animate-slide-up px-4">
            <img
              ...
              onLoad={() =>
                console.log("✅ Logo loaded successfully:", prepSmartLogo)
              }
              onError={(e) =>
                console.error("❌ Logo failed to load:", e, prepSmartLogo)
              }
            />
          </div>
          ...
          <div className="hidden md:flex items-center space-x-8">
            {navItems.map((item, index) => (
              <button
                ...
                className={`text-sm font-medium transition-all duration-300 hover:text-primary hover:scale-105 animate-slide-up ${
                  currentSection === item.id ||
                  (item.id === "practice" &&
                    currentSection === "practice-preview")
              ...
              {isAuthenticated ? (
                <div className="flex items-center space-x-3">
                  ...
                  <Button
                    ...
                    className="bg-card/80 border-border/50 hover:bg-card hover:border-border transition-smooth"
                  >
          ...
          <div className="md:hidden">
            <Button
              variant="ghost"
              size="sm"
              className="hover:bg-primary/10 transition-colors animate-scale-in"
            ...
            <div className="flex flex-col space-y-3">
              {navItems.map((item, index) => (
                <button
                  key={item.id}
                  onClick={() => handleNavClick(item.id)}
                  className={`text-left py-2 px-4 rounded-lg transition-all duration-300 hover:scale-105 animate-scale-in ${
                    currentSection === item.id ||
                    (item.id === "practice" &&
                      currentSection === "practice-preview")
                ...
                {isAuthenticated ? (
                  <div className="space-y-2">
                    ...
                    <Button
                      ...
                      size="sm"
                      onClick={handleSignOut}
                      className="w-full bg-card/80 border-border/50 hover:bg-card hover:border-border"
                    >
                      <LogOut className="h-4 w-4 mr-2" />
                      Sign Out
                    </Button>
